...
| 词库 | 权重来源 | 示例 |
|------|---------|------|
| `8105`（字表） | 文件内已带权重，**保留不动** | `的	de	6666` |
| `base`（核心） | 文件内已带权重，**保留不动** | `你好	ni hao	150` |
| `ext`（扩展） | 缺失的补为 100 | 脚本自动处理 |
| `tencent`（大词库） | 全部补为 100（原文件无权重） | 脚本自动处理 |
| `others`（杂项） | 文件内已带权重，**保留不动** | |
| **自定义词库** | 文件内自己设定，**保留不动** | 见下方示例 |

**自定义词库中直接写入权重**，构建时会保留：

```
我的词	wo de ci	500    # 权重 500，排序靠前
你的词	ni de ci	100    # 权重 100，排序靠后
```
