...
### 手动安装（Windows 小狼毫 / macOS 鼠须管）

> **推荐方式**：从 Release 下载 pre-built 包，无需本地构建。

1. 前往 [Release](https://github.com/josephxzy/med-ice/releases) 页面，下载最新的 `full.zip`
2. 解压后，将**压缩包内的所有文件**复制到 Rime 配置目录：
   - **Windows 小狼毫**：`%APPDATA%\Rime`
   - **macOS 鼠须管**：`~/Library/Rime`
   - **Linux iBus/Fcitx5**：`~/.config/ibus/rime` 或 `~/.local/share/fcitx5/rime`
3. 右键托盘图标 → **重新部署**

> 只需要更新词库的话，下载 `all_dicts.zip`，解压覆盖 `cn_dicts` `en_dicts` `opencc` 三个文件夹即可。

> 注意：多个文件可能与其他方案同名冲突。初次安装请**备份原先配置，清空配置目录**再导入。

### 开发者安装（从源码构建）

如果修改了词库或 Lua 脚本，需要先构建再安装：
...
> 项目重构后，plum 安装方式暂时不可用（文件已从仓库根目录迁移到 `src/`）。请使用 [手动安装](#手动安装windows-小狼毫--macos-鼠须管) 从 Release 页面下载。

<details>
<summary>℞ 安装或更新全部文件（初次安装先执行这个）</summary>

```bash
bash rime-install josephxzy/med-ice:src/recipes/full
```
...
```bash
bash rime-install josephxzy/med-ice:src/recipes/all_dicts
```

</details>

<details>
<summary>℞ 安装或更新中文词库</summary>

```bash
bash rime-install josephxzy/med-ice:src/recipes/cn_dicts
```

</details>

<details>
<summary>℞ 安装或更新英文词库</summary>

```bash
bash rime-install josephxzy/med-ice:src/recipes/en_dicts
```

</details>

<details>
<summary>℞ 安装或更新 Emoji</summary>

```bash
bash rime-install josephxzy/med-ice:src/recipes/opencc
```
...
bash rime-install josephxzy/med-ice:src/recipes/config:schema=double_pinyin
bash rime-install josephxzy/med-ice:src/recipes/config:schema=double_pinyin_mspy
bash rime-install josephxzy/med-ice:src/recipes/config:schema=double_pinyin_sogou
bash rime-install josephxzy/med-ice:src/recipes/config:schema=double_pinyin_abc
bash rime-install josephxzy/med-ice:src/recipes/config:schema=double_pinyin_ziguang
bash rime-install josephxzy/med-ice:src/recipes/config:schema=double_pinyin_jiajia
```
...
bash rime-install josephxzy/med-ice:src/recipes/grammar:schema=double_pinyin_sogou
bash rime-install josephxzy/med-ice:src/recipes/grammar:schema=double_pinyin_abc
bash rime-install josephxzy/med-ice:src/recipes/grammar:schema=double_pinyin_ziguang
bash rime-install josephxzy/med-ice:src/recipes/grammar:schema=double_pinyin_jiajia
```
...
bash rime-install josephxzy/med-ice:src/recipes/reverse_tone:schema=double_pinyin_sogou
bash rime-install josephxzy/med-ice:src/recipes/reverse_tone:schema=double_pinyin_abc
bash rime-install josephxzy/med-ice:src/recipes/reverse_tone:schema=double_pinyin_ziguang
bash rime-install josephxzy/med-ice:src/recipes/reverse_tone:schema=double_pinyin_jiajia
```
...
```sh
bash rime-install josephxzy/med-ice@2024.05.21:src/recipes/full
```
...
```bash
bash rime-install josephxzy/med-ice:src/recipes/no_lua_schema
```
...
2. 或者手动将 `src/no_lua_schema/` 目录下的文件粘贴并替换用户目录下文件。
