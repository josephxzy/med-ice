...
translator:
  dictionary: med_ice                    # ← 挂载 med_ice.dict.yaml
```

- `dictionary: med_ice` → Rime 自动在配置目录找 `med_ice.dict.yaml`
...
`build/rime/config.go` 中的 `DiscoverManifest()` 沿着这条链自动发现文件：
