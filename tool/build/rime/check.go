...
func initCheck() {
	...
	// 将所有读音读入 hanPinyin
	file3, err := os.Open(srcPath("8105"))
	if err != nil {
		log.Fatalln(err)
	}
	defer file3.Close()
...
func checkLine(dictPath string, _type int, line string, lineNumber int) {
	...
	// 除了字表，其他词库不应该含有单个的汉字
	if !isHanziDict(dictPath) && utf8.RuneCountInString(text) == 1 {
		fmt.Println("❌ 意外的单个汉字：", line)
	}

	// 除了 base，其他词库不应该含有两个字的词汇
	if !isBaseDict(dictPath) && utf8.RuneCountInString(text) == 2 {
	...
	if !isHanziDict(dictPath) && (_type == 2 || _type == 3) && !hanPinyinFilter.Contains(text) {
	...
	// 错别字检查，最耗时的检查
	if !isHanziDict(dictPath) && !wrongWordsFilter.Contains(text) {
