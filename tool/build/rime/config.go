...
func classifyDictFile(srcPath, name, category, outDir string) *DictFile {
	...
	if category == "cn" {
		...
		case "others":
			...
			df.NeedWeight = false
		default:
			if dc != nil && len(dc.Columns) > 0 {
				df.Columns = columnsToType(dc.Columns)
			} else {
				df.Columns = 3
			}
...
// columnsToType 将 YAML columns 声明转换为列数
func columnsToType(columns []string) int {
	if len(columns) == 0 {
		return 0
	}
