...
func initSchemas() {
	doublePinyin = schema{
		...
		path:            filepath.Join(OutDir, "en_dicts", "cn_en_double_pinyin.txt"),
		mapping: map[string]string{
			// 零声母
			"-a-":   "aa",
			"-e-":   "ee",
	...
	doublePinyinFlypy = schema{
		...
		desc:            "小鹤双拼",
		combinationType: "unique",
		path:            filepath.Join(OutDir, "en_dicts", "cn_en_flypy.txt"),
		mapping: map[string]string{
			// 零声母
			"-a-":   "aa",
			"-e-":   "ee",
	...
	doublePinyinMSPY = schema{
		...
		desc:            "微软双拼",
		combinationType: "unique",
		path:            filepath.Join(OutDir, "en_dicts", "cn_en_mspy.txt"),
		mapping: map[string]string{
			// 零声母
			"-a-":   "oa",
			"-e-":   "oe",
	...
	doublePinyinSogou = schema{
		...
		desc:            "搜狗双拼",
		combinationType: "unique",
		path:            filepath.Join(OutDir, "en_dicts", "cn_en_sogou.txt"),
		mapping: map[string]string{
			// 零声母
			"-a-":   "oa",
			"-e-":   "oe",
	...
	doublePinyinZiGuang = schema{
		...
		desc:            "紫光双拼",
		combinationType: "unique",
		path:            filepath.Join(OutDir, "en_dicts", "cn_en_ziguang.txt"),
		mapping: map[string]string{
			// 零声母
			"-a-":   "oa",
			"-e-":   "oe",
	...
	doublePinyinABC = schema{
		...
		desc:            "智能 ABC 双拼",
		combinationType: "unique",
		path:            filepath.Join(OutDir, "en_dicts", "cn_en_abc.txt"),
		mapping: map[string]string{
			// 零声母
			"-a-":   "oa",
			"-e-":   "oe",
	...
	doublePinyinJiajia = schema{
		...
		desc:            "拼音加加双拼",
		combinationType: "unique",
		path:            filepath.Join(OutDir, "en_dicts", "cn_en_jiajia.txt"),
		mapping: map[string]string{
			// 零声母
			"-a-":   "aa",
			"-e-":   "ee",
...
func CnEn() {
	...
	cnEnTXT, err := os.Open(filepath.Join(SrcDir, "dict", "en", "cn_en.txt"))
	if err != nil {
		log.Fatalln(err)
	}
	defer cnEnTXT.Close()
	...
	schemas := []schema{
		{name: "cn_en", desc: "全拼", combinationType: "unique", path: filepath.Join(OutDir, "en_dicts", "cn_en.txt")},
		doublePinyin,
		doublePinyinFlypy,
		doublePinyinMSPY,
		doublePinyinSogou,
...
#@/db_type	tabledb
#
# https://github.com/josephxzy/med-ice
# ------- 中英混输词库 for %s -------
# 由 src/dict/en/cn_en.txt 自动生成
#
# 此行之后不能写注释
`, s.name, s.desc)
