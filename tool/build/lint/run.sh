...
find_business_yaml_files() {
  (
    cd "${REPO_ROOT}"
    {
      ...
      find ./src/config \
        -maxdepth 1 -type f \( -name "*.yaml" -o -name "*.yml" \) -print
      find ./src/no_lua_schema \
        -maxdepth 1 -type f \( -name "*.yaml" -o -name "*.yml" \) ! -name "*.dict.yaml" -print
      find ./src/recipes \
        -maxdepth 1 -type f \( -name "*.yaml" -o -name "*.yml" \) -print
    }
...
run_lua_lint() {
  ...
  log_step 'lua-lint checking src/lua/**/*.lua'

  set +e
  lint_output="$(
    (
      cd "${REPO_ROOT}"
      luacheck src/lua --no-default-config --config "${LUACHECK_CONFIG}" --codes --ranges
    ) 2>&1
  )"
  lint_status=$?
  set -e
...
usage() {
  cat <<'EOF'
Usage: bash build/lint/run.sh <command>
